...
Este script es parte de un proyecto académico para la materia de Diseño Funcional (FESC).
Su objetivo es demostrar el uso de variables, colecciones y funciones en Python aplicadas a un escenario realista de gestión veterinaria.

Estructura y propósito:
- Cada sección del código corresponde a una actividad didáctica (variables, listas, operadores, lógica de negocio).
- Las funciones están diseñadas para ilustrar conceptos fundamentales de programación funcional y buenas prácticas.
- Se eligió esta estructura modular para facilitar la comprensión, reutilización y evaluación de cada componente por separado.
...
def normalize_veterinary_text(text: str) -> str:
    ...
    # Se usa strip() y lower() para asegurar que el texto sea uniforme y fácil de comparar,
    # lo cual es esencial en tareas de procesamiento de lenguaje natural (NLP) y búsqueda.
    # Esta función es clave para evitar errores por mayúsculas/minúsculas o espacios accidentales.
...
def calculate_total_service_cost(
    ...
    # Se utiliza esta función para centralizar el cálculo de precios finales,
    # evitando duplicación de lógica y posibles errores en diferentes partes del sistema.
    # Así, cualquier cambio en la fórmula de impuestos se realiza en un solo lugar.
...
def is_clinic_open(current_time: str) -> bool:
    ...
    # Se usa comparación de strings en formato HH:MM porque es suficiente para horarios simples
    # y evita la necesidad de convertir a objetos datetime, simplificando el código para el caso académico.
    # El uso de not permite excluir el horario de almuerzo fácilmente.
...
def can_schedule_appointment(
    ...
    # Se usa el operador lógico and para asegurar que solo si todas las condiciones son verdaderas
    # se permita agendar la cita. Esto refleja reglas de negocio típicas en sistemas de gestión.
...
# GLOSARIO
# ============================================================================
# - Variable global: Variable definida fuera de funciones, accesible en todo el script.
# - Colección: Estructura de datos que agrupa varios elementos (ej: lista, diccionario).
# - Función pura: Función que no modifica variables externas y siempre retorna el mismo resultado para los mismos argumentos.
# - Operador lógico: Símbolo que permite combinar condiciones (ej: and, or, not).
# - Docstring: Comentario especial al inicio de funciones/módulos que explica su propósito y uso.
# - Modularidad: Organización del código en bloques independientes y reutilizables.
...
# - Documentar cada función con docstrings claros y ejemplos.
# - Evitar duplicar lógica; centralizar cálculos y validaciones en funciones reutilizables.
# - Separar datos de lógica cuando el proyecto crezca (usar módulos o clases).
# - Seguir la convención PEP 8 para estilo y legibilidad.
# - Comentar el propósito de bloques complejos o decisiones de diseño.
